...
function getSubCategorias(id,idCategoria) {
    ...
    $.ajax({
            ...
            url: location,
        success: function (respuesta) {
            var renglon = "<option id='0'  value=''>Seleccione una Subcategoria...</option> ";
            respuesta.forEach(function(row, index) {
...
function getServicios(id) {
    ...
    $.ajax({
            ...
            url: location,
        success: function (respuesta) {
            var renglon = "<option id='0'  value=''>Seleccione un tipo Servicio...</option> ";
            respuesta.forEach(function(row, index) {
...
function getProveedores(id) {
    ...
    $.ajax({
            ...
            url: location,
        success: function (respuesta) {
            var renglon = "<option id='0'  value=''>Seleccione un Proveedor...</option> ";
            respuesta.forEach(function(row, index) {
...
function getProductosTable() {
    ...
    $.ajax({
            ...
            _success: function (respuesta) {
                ...
                respuesta.forEach(function (row, index) {
                    renglon = "<tr>"
                        + "<td class='dtr-control'>" + row.prd_id + "</td>"
                        + "<td>" + row.prd_name + "</td>"
                        + "<td>" + row.prd_english_name + "</td>"
                        + "<td>" + row.prd_sku + "</td>"
                        + "<td>" + row.prd_model + "</td>"
                        + "<td>" + row.prd_serial_number + "</td>"
                        + "<td>" + row.prd_cost + "</td>"
                        + "<td>" + row.prd_price + "</td>"
                        ...
                        + '<button onclick="EditProducto(' + row.prd_id +','+ row.cat_id +','+ row.sbc_id +','+ row.srv_id +','+ row.str_id +','+ row.prd_coin_type +','+ row.sup_id +','+ row.prd_visibility +','+ row.stp_id +')" type="button" class="btn btn-default btn-icon-edit" aria-label="Left Align"><i class="fas fa-pen"></i></button>'
                        + '<button onclick="ConfirmDeletProducto(' + row.prd_id + ')" type="button" class="btn btn-default btn-icon-delete" aria-label="Left Align"><i class="fas fa-trash"></i></button>'
                        + '</td>'
                        + "</tr>";
                 ...
                 table = $('#ProductosTable').DataTable({
                    ...
                    { extend: 'pageLength', className: 'btnDatableAdd' },
                    {
                        ...
                        action: function () {
                            ...
                            if (idSelected != "") { ConfirmDeletProducto(idSelected); }
                        }
                    }
                    ],
                    columnDefs: [
